...
1.	The first Blast (blastx) was done using the Arabidopsis thaliana protein sequences against the S. Pombe DNA db. Best hits IDs (e-value <= 1.10-6) were saved and their sequences were retrieved from the S. Pombe db, to be used in the next blast. (**)

2.	The second Blast (tblastn) was done using protein best hits from the previous step against Arabidopsis genes DNA db, with the same e-value threshold. 
...
**: Note: when I run the whole Arabidopsis seq data set against S.Pombe db, I got an error message that interrupted the code : 
...
I mentioned that error in my email, and I checked the type of Blast I suggested, but Blast (blastx) works fine for shorter sequences enquires against the protein db. Therefore, I run the code using a shorter version of Arabidopsis list of genes to probe that it works.
